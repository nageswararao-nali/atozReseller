...
const Products = ({ route, fetchSubCategoriesData, fetchSubCategoriesProducts, isLoading, subCategories, categoryProducts, mobile, cartData, updateCart, bookOrderFn, orderDocumentUrl, clearCartFn }) => {
  ...
  useEffect(() => {
    if(categoryProducts) {
      console.log("sub categories products in products page.......", categoryProducts)
      let cats = categoryProducts.filter((cats) => {
        return cats.id == categoryId
      })
      setCategory(cats[0])
    }
  }, [categoryProducts])
                ...
                renderItem={({item}) => (
                  <View>
                    ...
                    <View style={{flexDirection: 'row', marginBottom: 10}}>
                      ...
                      <View style={{width: '50%'}}>
                        <View>
                          <Text style={{fontWeight: 'bold'}}>{item.name + ' - ' + item.items[0].item.name + " " + item.items[0].quantity}</Text>
                        </View>
                        <View>
                          <Text style={{}}>{'Price: 555' }</Text>
                        </View>
                      </View>
                      <View style={{width: '20%'}}>
                        <InputSpinner
                        max={item.items[0].quantity}
                        min={0}
                        step={1}
                        skin={"round"}
                        ...
                        value={item.items[0].value ? item.items[0].value : 0}
                        onChange={num => numChangeEvent(num, item.items[0].item.id, category.id, item.id, category.name, item.name, item.items[0].item.name)}
                    ...
                    :
                    */ }
                    <TouchableOpacity onPress={() => navigate('Items', {product: item, categoryId: category.id, categoryName: category.name})}>
                      <View style={{flexDirection: 'row', marginBottom: 10}}>
                          <View style={{width: '20%'}}>
                            <Image style={{width: 50, height: 50, borderRadius: 10}}  source={{uri: item.image}} />
                          </View>
